...
import { SearchIcon, FilterIcon, PlusIcon, DownloadIcon, PdfIcon, CsvIcon, ExcelIcon } from './icons';

export const HrView: React.FC = () => {
    const { employees: allEmployees, loading, branches, currentBranchId } = useSystem();
    ...
    const employees = useMemo(() => {
        if (!currentBranch) return allEmployees;
        return allEmployees.filter(e => e.branch === currentBranch.name);
    }, [allEmployees, currentBranch]);
    ...
    const headers = [t('name'), t('position'), t('branch'), t('salary'), t('status')];
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-foreground">{t('employee_management')}</h1>
                    <p className="mt-2 text-muted-foreground">{t('employee_subtitle')}</p>
                </div>
                <button
                    className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 w-full sm:w-auto"
                >
                    <PlusIcon />
                    {t('add_employee')}
                </button>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                    ...
                    <div className="relative group">
                        ...
                        <div className="absolute top-full ltr:right-0 rtl:left-0 mt-2 w-40 bg-card border border-border rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto z-10">
                            <button onClick={() => exportToPdf(dataForExport, headers, t('employee_management'), 'employees_report')} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-secondary text-sm"><PdfIcon/> {t('export_as_pdf')}</button>
                            <button onClick={() => exportToCsv(dataForExport, 'employees_report')} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-secondary text-sm"><CsvIcon/> {t('export_as_csv')}</button>
                            <button onClick={() => exportToExcel(dataForExport, 'employees_report')} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-secondary text-sm"><ExcelIcon/> {t('export_as_excel')}</button>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full text-sm text-left rtl:text-right text-foreground">
                        <thead className="text-xs text-muted-foreground uppercase bg-secondary">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">{t('salary')}</th>
                            ...
                            {loading ? (
                                [...Array(5)].map((_, i) => (
                                    <tr key={i} className="bg-card border-b border-border animate-pulse">
                                        ...
                                        <td className="px-6 py-4"><div className="h-4 bg-secondary rounded w-2/3"></div></td>
                                        <td className="px-6 py-4"><div className="h-4 bg-secondary rounded w-1/3"></div></td>
                                        <td className="px-6 py-4"><div className="h-6 w-20 bg-secondary rounded-full"></div></td>
                                        <td className="px-6 py-4"><div className="h-4 bg-secondary rounded w-1/4"></div></td>
                                    </tr>
                                ))
                            ) : (
                                filteredEmployees.map(emp => (
                                    <tr key={emp.id} className="bg-card border-b border-border hover:bg-secondary transition-colors">
                                        <td className="px-6 py-4 font-medium whitespace-nowrap">{emp.name}</td>
                                        <td className="px-6 py-4">{emp.position}</td>
                                        <td className="px-6 py-4">{emp.branch}</td>
                                        <td className="px-6 py-4">EGP {emp.salary.toLocaleString()}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${getStatusColor(emp.status)}`}>
                                                {emp.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
